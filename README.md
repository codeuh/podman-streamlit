...
2. The Container Stats page is assuming the network name. Need to make that dynaimc and selectable.
3. I had to disable SELinux by running the below command when I was testing it on Fedora 41 Workstation. Might there be a better way to hanlde this? I'm not SELinux expert.
````bash
sudo setenforce 0
````
4. You need to make sure the podman socket you are mounting in is enabled and running. Check the commands below for enabling your users rootless socket.
````bash
